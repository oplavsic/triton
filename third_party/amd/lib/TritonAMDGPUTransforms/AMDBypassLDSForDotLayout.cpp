...
//===----------------------------------------------------------------------===//
// AMDBypassLDSForDotLayoutPass Overview
//
// The AMDBypassLDSForDotLayoutPass implements a strategy to bypass using the
...
public:
  ...
  bool isEligibleConvertOp(ttg::ConvertLayoutOp convertOp) {
    ...
    if (!srcType || !dstType || !srcType.isF16() ||
        srcType.getShape().size() == 2)
      return false;
